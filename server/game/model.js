...
let pictures = [];
picture_loader().then(x => (pictures = x));
let iCaptions = 0;
let iPicture = 0;
...
class Game {
  ...
  getPlayedCaptions() {
    if (this.playedCaptions.some(x => x.isChosen)) {
      return this.playedCaptions.map(x => ({
        ...x,
        playerName: this.players[x.playerId].name
      }));
    } else {
  ...
  flipPicture(playerId) {
    if (!this.isDealer(playerId)) {
      throw new Error("Only the dealer can flip a picture");
    }
  ...
  submitCaption(playerId, text) {
    if (this.isDealer(playerId)) {
      throw new Error("The dealer cannot submit a caption");
    }
    this.playedCaptions.push({
      playerId: playerId,
      text: text,
      isChosen: false
    });
  ...
  chooseCaption(playerId, text) {
    if (!this.isDealer(playerId)) {
      throw new Error("Only the dealer can choose a caption");
    }
